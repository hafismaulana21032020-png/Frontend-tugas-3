...
// Setup Endpoint GET dan Penangkapan Parameter (Paginasi)
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Eksekusi Pengambilan Data dan Pembungkusan Respons Sukses
    const result = await getTasks({ limit, skip });
    const latencyMs = Date.now() - startTime;

    const responsePayload: ApiResponse = {
      success: true,
      message: 'Konektsi ke DummyJSON API berhasil! Data berhasil diambil.',
    ...
    // Tambahkan penanganan Error yang Aman (GET)
    const latencyMs = Date.now() - startTime;
...
// Setup Endpoint POST dan Validasi Payload Dasar
export async function POST(request: NextRequest) {
  try {
    ...
    // Eksekusi Pembuatan Data (Simulasi) dan Pengiriman Respons
